...
$json = (Get-Content ".\ScaleDownCosmos\resources.json" -Raw) | ConvertFrom-Json
...
foreach($item in $json.resources){
    ...
    $throughputScaleDown = $item.throughputScaleDown
    ...
    Write-Host "throughputScaleDown = $throughputScaleDown"
    ...
    $Newthroughput = $throughputScaleDown -as [Int32]
    ...
    switch($api){
        "sql"{
            if($isDedicatedThroughput){ #container level throughput
                ...
                if($minThroughput -lt $Newthroughput)
                {
    ...
    Write-Host "Throughput scaled down on resource $resourceName in account $accountName using $throughputType from $currentThroughput RU/s to $Newthroughput RU/s`n`n"

}

# Write an information log with the current time.
Write-Host "Cosmos DB ScaleDownTrigger ran! TIME: $currentUTCtime"
$body = "Cosmos DB ScaleDownTrigger ran successfully! TIME: $currentUTCtime"
